...
orbit_fates = ['trivial', 'exhaustible', 'inexhaustible', 'identity']
...
# all remaining 8 cells are structurally forbidden
forbidden_cells = {(i, j) for i in range(3) for j in range(4)} - set(occupied.keys())
...
for i in range(3):
    for j in range(4):
        ...
        if key in occupied:
            ...
            # draw X
            ax.plot([-0.7, 0.7], [-0.7, 0.7], color='#2a1a1a', lw=1.2, alpha=0.8)
            ax.plot([-0.7, 0.7], [0.7, -0.7], color='#2a1a1a', lw=1.2, alpha=0.8)
...
fig.text(0.55, 0.025,
         'the 3×4 looks like space. it isn\'t. four cells, not twelve.',
         color='#444455', fontsize=9, ha='center', va='center')

plt.savefig('assets/constrained-matrix.png', dpi=150, bbox_inches='tight',
            facecolor='#080808')
print("saved constrained-matrix.png")
